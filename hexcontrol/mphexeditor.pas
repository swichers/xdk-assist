...
const
  ...
  MPHTranslationDesc: array[TMPHTranslationKind] of string = (MPH_TK_ASIS,
    MPH_TK_DOS8, MPH_TK_ASCII7, MPH_TK_MAC, MPH_TK_BCD38,
    MPH_TK_CUSTOM);
...
const
  MPH_VERSION = 'December 29, 2004; © markus stephany, vcl[at]mirkes[dot]de' ;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (LWStrOutput[1] < #256) and (Char(LWStrOutput[1]) in FMaskedChars) then
...
;
    ...
    if (LIntCurRow = Row) and LBoolFocused and (GetOtherFieldColCheck(Col) =
      LIntCurCol) then
    begin
      with LRctWhere do
        if FFocusFrame then
...
begin
{$IFDEF FASTACCESS}
  //FDataStorage.CheckBounds(Index+1);
{$R-}
  Result := GetFastPointer^[Index];
{$ELSE}
...
begin
{$IFDEF FASTACCESS}
  //FDataStorage.CheckBounds(Index+Count);
  Move(GetFastPointer^[Index], Buffer, Count);
{$ELSE}
...
{$IFDEF FASTACCESS}
  //FDataStorage.CheckBounds(Index+Count);
  Move(Buffer, GetFastPointer^[Index], Count);
{$ELSE}
...
begin
  // auswahl berechnen
  LBoolInCharField := GetInCharField;
  if DataSize - APosition < aOldCount then
  begin
    if aNewCount = aOldCount then
      aNewCount := DataSize - APosition;
    aOldCount := DataSize - APosition;
  end;
  if IsFileSizeFixed then
  begin
  ...
  begin
    if aOldCount > 0 then
      InternalDelete(APosition, APosition + aOldCount, Col, Row);
    if aNewCount > 0 then
      InternalInsertBuffer(aBuffer, aNewCount, APosition);
  end;
  Result := aNewCount;
...
function TCustomMPHexEditor.GetFastPointer: PByteArray;
begin
...
{$IFDEF FASTACCESS}
      if Value > iSize then
        ...
        FillChar(FastPointer^[iSize], Value - iSize, FSetDataSizeFillByte);
{$ENDIF}
      FModified := True;
      CalcSizes;
...
var
  ...
  begin
    ...
    LPurUndoRec := PMPHUndoRec(@(PChar(Memory)[i]));

    FillChar(LPurUndoRec^, SizeOf(TMPHUndoRec) + ASize, 0);
    with LPurUndoRec^ do
    begin
      Flags := [aKind];
  ...
  begin
    ...
    if ufFlagHasSelection in LPurUndoRec^.Flags then
      Inc(LPurUndoRec^.DataLen, sizeof(TUndoSelRec));
    if ufFlagHasDescription in LPurUndoRec^.Flags then
      Inc(LPurUndoRec^.DataLen, system.Length(SDescription) + sizeof(i));
  ...
  begin
    ...
    case aKind of
      ...
      ufKindNibbleInsert:
        begin
          NewFillBuffer(0);
          LPurUndoRec.Buffer := FEditor.Data[aPosition];
          if FEditor.HasChanged(aPosition) then
            Include(LPurUndoRec.Flags, ufFlagByte1Changed);
          UpdateUndoRecord;
        end;
